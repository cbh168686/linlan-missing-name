...
function effigyPlate(){return `<section class="horror-exhibit effigy-exhibit" data-effigy-side="front" aria-label="守灯像器物留影与背页"><div class="exhibit-bar"><span>陈列像 A / 器物组留影</span><span>原名位：空</span></div><div class="effigy-collage">${pastedEyes(36)}<img class="effigy-photo" src="./assets/lamp-effigy-v1.png" alt="六只木手围着胸口空名牌槽的旧金色守灯像，四周贴满黑白眼睛"><span class="effigy-strip strip-left" aria-hidden="true">不问旧姓　不认旧人</span><span class="effigy-strip strip-right" aria-hidden="true">有目无名　有名无声</span><div class="effigy-reverse"><span>背 页 压 字</span><strong>像没有睁眼。<br>是纸上的眼睛<br>在替它看。</strong><p>原名位不可空置。<br>若未收到名牌，<br>先填答应的人。</p></div><span class="effigy-number">A—017 / 不得裁去页边</span></div><p class="exhibit-caption" data-effigy-note aria-live="polite">名牌槽是空的。四周的眼睛有相同的剪口，像是从同一张纸上反复剪下来。</p><div class="exhibit-controls"><span>留影 / 附背页</span><button data-effigy-flip>翻到背页</button></div></section>`;}
...
function identityPlate(){return `<section class="horror-exhibit identity-exhibit" data-identity-step="0" aria-label="玩家失踪后的联系记录"><div class="exhibit-bar"><span>校友联系表 / 夜间替换记录</span><span>更正申请已受理</span></div><div class="identity-screen"><div class="identity-count"><span>2018 届七班</span><strong data-contact-count>35</strong><small>当前可联系人数</small></div><div class="identity-card"><div class="erased-portrait" aria-hidden="true"><span></span></div><div><small>最后一条联系记录</small><strong data-erased-name>你</strong><span data-identity-status>正在核对</span></div></div><div class="identity-message"><span>周栩 → 你</span><p>到家了吗？回我一句。</p></div><div class="identity-reply"><span>你的账号 → 周栩</span><p>别查了，都是误会。我想安静一阵。</p><small>已发送 · 你没有碰过那部手机</small></div><p class="identity-caption" data-identity-note aria-live="polite">你看着自己的那一栏，等待页面加载完。</p></div><div class="exhibit-controls"><span data-identity-progress>记录 01 / 04</span><button data-identity-next>继续读取</button></div></section>`;}
...
function chantPlate(){return `<section class="horror-exhibit chant-exhibit" data-chant-step="0" aria-label="归灯诵读页"><div class="exhibit-bar"><span>守灯会 · 归灯前七夜</span><button class="ritual-cover-button" data-ritual-open>展开封面</button></div><div class="chant-paper"><div class="chant-echo" aria-hidden="true">借名　留灯　候归<br>借名　留灯　候归<br>借名　留灯　候归</div><span class="chant-counter">第 一 遍</span><div class="chant-lines" aria-live="polite">${ritualLines[0].map(t=>`<p>${t}</p>`).join('')}</div><div class="chant-answer" aria-hidden="true">在</div></div><div class="exhibit-controls"><span data-chant-note>末尾须停一次</span><button data-chant-next>往下读</button></div></section>`;}
const ritualShown=new Set(),corruptionShown=new Set();
let corruptionTimer=null,restoreCorruption=null;
function clearDread(){clearTimeout(corruptionTimer);if(restoreCorruption)restoreCorruption();document.querySelector('.ritual-interruption')?.close();}
...
function setupEyes(){
  ...
  document.body.classList.toggle('uncanny-catalogue',state.horror&&(eyes.length+scenes.length)>0);
  ...
  scenes.forEach(el=>{const kind=el.dataset.horrorScene;el.innerHTML=!state.horror?coveredHorror('恐怖场景'):kind==='corridor'?corridorPlate():kind==='rollcall'?rollcallPlate():kind==='identity'?identityPlate():kind==='chant'?chantPlate():kind==='effigy'?effigyPlate():'';});
}
...
document.addEventListener('click',e=>{
  ...
  const identity=e.target.closest('[data-identity-next]');
  if(identity){const box=identity.closest('.identity-exhibit'),n=(+box.dataset.identityStep+1)%4;box.dataset.identityStep=n;box.querySelector('[data-contact-count]').textContent=n>=2?'34':'35';box.querySelector('[data-erased-name]').textContent=n>=2?'查无此人':'你';box.querySelector('[data-identity-status]').textContent=['正在核对','已读 · 未回复','按本人意愿移除','院内称呼：待归'][n];box.querySelector('[data-identity-note]').textContent=['你看着自己的那一栏，等待页面加载完。','消息来了。手机在门外的人手里。','照片和名字消失了。人数少了一个。','门外开始点名。你知道下一声不会叫你的名字。'][n];box.querySelector('[data-identity-progress]').textContent=`记录 0${n+1} / 04`;identity.textContent=n===3?'重新查看':'继续读取';}
});
